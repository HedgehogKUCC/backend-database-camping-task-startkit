...
UPDATE "USER"
SET role = 'COACH'
WHERE email IN ( SELECT email FROM "USER" WHERE name IN ('李燕容', '肌肉棒子', 'Q太郎') );
...
VALUES
    (
        ( SELECT c.id FROM "COACH" c JOIN "USER" u ON u.id = c.user_id WHERE u.name = '李燕容' ),
        ( SELECT id FROM "SKILL" WHERE name = '重訓' )
    ),
    (
        ( SELECT c.id FROM "COACH" c JOIN "USER" u ON u.id = c.user_id WHERE u.name = '肌肉棒子' ),
        ( SELECT id FROM "SKILL" WHERE name = '重訓' )
    ),
    (
        ( SELECT c.id FROM "COACH" c JOIN "USER" u ON u.id = c.user_id WHERE u.name = 'Q太郎' ),
        ( SELECT id FROM "SKILL" WHERE name = '重訓' )
    ),
    (
        ( SELECT c.id FROM "COACH" c JOIN "USER" u ON u.id = c.user_id WHERE u.name = '肌肉棒子' ),
...
SELECT c_purchase.user_id, SUM(c_package.credit_amount) total
FROM "CREDIT_PURCHASE" c_purchase
         JOIN "USER" u ON c_purchase.user_id = u.id
         JOIN "CREDIT_PACKAGE" c_package ON c_purchase.credit_package_id = c_package.id
WHERE u.name = '王小明'
GROUP BY c_purchase.user_id;

-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
SELECT cb.user_id, COUNT(*)
FROM "COURSE_BOOKING" cb
         JOIN "USER" u ON u.id = cb.user_id
WHERE u.name = '王小明' AND cb.join_at IS NOT NULL
GROUP BY cb.user_id;
...
SELECT u.id AS user_id, (
                            SELECT SUM(c_package.credit_amount) total_credit
                            FROM "CREDIT_PURCHASE" c_purchase
                                     JOIN "USER" u ON c_purchase.user_id = u.id
                                     JOIN "CREDIT_PACKAGE" c_package ON c_purchase.credit_package_id = c_package.id
                            WHERE u.name = '王小明'
...
FROM "USER" u
         ...
         JOIN "COURSE_BOOKING" cb ON u.id = cb.user_id
WHERE name = '王小明'
GROUP BY u.id;
...
SELECT U.name 教練名稱, C.experience_years 經驗年數, S.name 專長名稱
FROM "USER" U
         JOIN "COACH" C ON U.id = C.user_id
         JOIN "COACH_LINK_SKILL" CLS ON CLS.coach_id = C.id
...
SELECT S.name 專長名稱, COUNT(CLS.coach_id) coach_total
...
SELECT CPA.name 組合包方案名稱, COUNT(CP.credit_package_id) 銷售數量
FROM "CREDIT_PURCHASE" CP
         JOIN "CREDIT_PACKAGE" CPA ON CP.credit_package_id = CPA.id
WHERE TO_CHAR(CP.purchase_at, 'YYYY-MM') = '2024-12'
GROUP BY CPA.name;

-- 6-4. 查詢：計算 12 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
SELECT SUM(CPA.price) 總營收
FROM "CREDIT_PURCHASE" CP
         JOIN "CREDIT_PACKAGE" CPA ON CP.credit_package_id = CPA.id
WHERE TO_CHAR(CP.purchase_at, 'YYYY-MM') = '2024-12';
